  ...
  const handleFetchDetails = async () => {
    ...
    try {
      ...
      const thumbnailResponse = await axios.post(
        "http://localhost:8000/api/v1/video/generator",
        {
          videoUrl: videoUrl,
          frameCount: 3,
        }
      ...
      console.log(
        "risyandi ~ handleFetchDetails ~ thumbnailResponse:",
        thumbnailResponse
      );
      const metadataResponse = await axios.post(
        "http://localhost:8000/api/v1/video/extractor",
        {
          videoUrl: videoUrl,
        }
      );
      console.log(
        "risyandi ~ handleFetchDetails ~ metadataResponse:",
        metadataResponse
      );
      ...
      setError(
        "Failed to generate video. Please check the URL or try again."
      );
    }
  };
        ...
        <div className="max-w-lg mx-auto bg-white shadow-md rounded-lg p-6">
          ...
          {metadata && (
            <div className="mt-6">
              ...
              <textarea
                className="bg-gray-100 p-4 mt-2 text-sm resize-none rounded-md w-full h-52"
